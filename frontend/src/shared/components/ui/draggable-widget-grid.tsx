...
import styles from './draggable-widget-grid.module.css';
...
const DEFAULT_ITEMS: WidgetItem[] = [
    { id: 'widget-1', size: 'wide' }, { id: 'widget-2', size: 'sm' },
    { id: 'widget-3', size: 'sm' }, { id: 'widget-4', size: 'sm' },
    { id: 'widget-5', size: 'wide' }, { id: 'widget-6', size: 'sm' },
    { id: 'widget-7', size: 'sm' }, { id: 'widget-8', size: 'wide' },
];
...
interface Placement { id: string; col: number; row: number; w: number; h: number; }
interface Box { col: number; row: number; w: number; h: number; }

const overlaps = (a: Box, b: Box) => a.col < b.col + b.w && b.col < a.col + a.w && a.row < b.row + b.h && b.row < a.row + a.h;
const contains = (outer: Box, inner: Box) => inner.col >= outer.col && inner.row >= outer.row && inner.col + inner.w <= outer.col + outer.w && inner.row + inner.h <= outer.row + outer.h;
function spanOf(item: WidgetItem, columns: number) { return { w: Math.min(SPANS[item.size].col, columns), h: SPANS[item.size].row }; }
...
interface Slot { left: number; top: number; right: number; bottom: number; }
interface Candidate { order: WidgetItem[]; slot: Slot; }
const ENTER = 0.18;
...
function candidatesFor(items: WidgetItem[], id: string, columns: number, toSlot: (box: Box) => Slot): Candidate[] {
    ...
    for (let row = 0; row + me.h <= rows; row++) {
        for (let col = 0; col + me.w <= columns; col++) {
            ...
            out.push({ order: moved.map((p) => byId.get(p.id) as WidgetItem), slot: toSlot(area) });
        }
    }
...
const SHADOW_LIFTED = '0px 28px 60px -16px rgba(0,0,0,0.65), 0px 10px 24px -8px rgba(0,0,0,0.5)';

interface Press { timer: number; pointerId: number; x: number; y: number; }
...
const Widget = memo(function Widget({
    item, col, row, w, h, columns, rows, editable, held, raised, landed, handlers, hintId, position, count, renderItem,
}: {
    item: WidgetItem; col: number; row: number; w: number; h: number; columns: number; rows: number;
    editable: boolean; held: boolean; raised: boolean; landed: boolean; handlers: WidgetHandlers;
    hintId: string; position: number; count: number; renderItem?: (item: never, size: WidgetSize) => ReactNode;
}) {
    ...
    const onPointerDown = (e: ReactPointerEvent) => {
        ...
        if (e.pointerType !== 'touch') { controls.start(e); return; }
        if (press.current || lifted.current) return;
        const origin = e.nativeEvent;
        const pointerId = e.pointerId;
        setPhase('holding');
        press.current = {
            pointerId, x: e.clientX, y: e.clientY,
    ...
    return (
        <motion.div
            ...
            onContextMenu={(e) => { if (phase !== 'idle') e.preventDefault(); }}
            ...
            transition={SPRING}
            className={`${styles.widget} ${editable ? styles.widgetEditable : ''}`}
            style={{
            ...
            <motion.div
                ...
                className={`${styles.widgetInner} ${landed ? styles.widgetInnerLanded : styles.widgetInnerNormal}`}>
...
export default function DraggableWidgetGrid({
    ...
    useIsoLayoutEffect(() => { latest.current.items = items; }, [items]);
    ...
    const toSlot = useCallback((box: Box): Slot => {
        const el = grid.current;
    ...
    const step = useCallback((force = false) => {
        frame.current = 0;
    ...
    useEffect(() => () => { cancelAnimationFrame(frame.current); window.clearTimeout(ring.current); }, []);
    ...
    const handlers: WidgetHandlers = useMemo(() => ({
        start: (id) => {
            ...
            setHeld(id);
            setRaised(id);
            window.addEventListener('pointerup', () => { swallowUntil.current = performance.now() + 300; }, { once: true, capture: true });
        },
        drag: () => { if (!frame.current) frame.current = requestAnimationFrame(() => step()); },
        end: (id) => {
            ...
            ring.current = window.setTimeout(() => { setLanded(null); setRaised(null); }, LANDED_MS);
        ...
        suppressClick: () => { swallowUntil.current = performance.now() + 300; },
    ...
    return (
        <MotionConfig reducedMotion="user">
            <div className={`${styles.gridWrapper} ${className}`} style={{ '--widget-radius': `${radius}px` } as CSSProperties}>
                {editable && (
                    <p id={hintId} className={styles.srOnly}>
                        Drag to rearrange. On touch screens, press and hold first. With a keyboard, hold Alt and press the arrow keys.
                    </p>
                )}
                <div
                    ref={grid}
                    ...
                    {domOrder.current.map((id) => {
                        ...
                        return (
                            <Widget
                                key={id} position={(visualIndex.get(id) ?? 0) + 1} count={placements.length}
                                item={item} col={p.col} row={p.row} w={p.w} h={p.h} columns={columns} rows={rows}
                                editable={editable} held={held === p.id} raised={raised === p.id} landed={landed === p.id}
                                handlers={handlers} hintId={hintId} renderItem={renderItem as (item: never, size: WidgetSize) => ReactNode}
                            />
                        );
                    })}
                </div>
